...
${skills || 'Grundlegende EDV-Kenntnisse, Teamfähigkeit'}
...
${ausbildungTarget}${companyName ? ` bei ${companyName}` : ''}
` : ''}


${address ? address.split(',').pop().trim() : 'Ort'}, ${new Date().toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' })}
...
export function generateBewerbungsschreiben(data) {
  ...
  let body1 = '';
  if (eduLevel) {
    body1 = `Zurzeit ${eduLevel.includes('Abitur') || eduLevel.includes('Studium') ? 'habe ich' : 'verfüge ich über'} ${eduLevel}${schoolName && schoolName.trim() ? ` an ${schoolName}` : ''}. `;
  }
